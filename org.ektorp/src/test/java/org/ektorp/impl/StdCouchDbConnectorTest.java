...
public class StdCouchDbConnectorTest {
	...
	public void testCreate() throws IOException {
		...
		doReturn(HttpResponseStub.valueOf(201, OK_RESPONSE_WITH_ID_AND_REV))
	...
	@Test
	public void docId_should_be_escaped_when_id_contains_slash()
	...
	public void testCreateFromJsonNode() throws Exception {
		...
		JsonNode root;
		{
			...
			try {
				...
				root = new ObjectMapper().readValue(resourceAsStream,
						JsonNode.class);
			} finally {
	...
	public void save_document_with_utf8_charset() throws IOException {
		...
		ArgumentCaptor<String> ac = ArgumentCaptor.forClass(String.class);
		verify(httpClient).put(eq("/test_db/some_id"), ac.capture());
		assertEqualJson("charset.json", Charset.forName("UTF-8"),
				ac.getValue());
	}
	...
	public void save_document_with_ISO_8859_1_charset() throws IOException {
		...
		ArgumentCaptor<String> ac = ArgumentCaptor.forClass(String.class);
		verify(httpClient).put(eq("/test_db/some_id"), ac.capture());
		assertEqualJson("charset-ISO-8859-1.json",
				Charset.forName("ISO-8859-1"), ac.getValue());
	}
	...
	public void create_should_post_if_id_is_missing() throws IOException {
		...
		assertEquals("some_id", td.getId());
		assertEquals("123D123", td.getRevision());
		assertEqualJson("create_with_no_id.json", Charset.forName("UTF-8"),
				ac.getValue());
	}
	...
	public void testDelete() {
		doReturn(HttpResponseStub.valueOf(200,
				"{\"ok\":true,\"rev\":\"123D123\"}")).when(httpClient)
						.delete(anyString());
	...
	private void setupGetDocResponse(String... ids) {
		for (String id : ids) {
			doReturn(HttpResponseStub.valueOf(200,
					"{\"name\":\"nisse\",\"age\":12,\"_id\":\" " + id
							+ "\",\"_rev\":\"123D123\"}")).when(httpClient)
									.get(TEST_DB_PATH + id);
		}
	}
	...
	public void throw_exception_when_create_attachment_in_conflict() {
		...
		td.setRevision("123D123");
		doReturn(ResponseOnFileStub.newInstance(409, "update_conflict.json"))
				.when(httpClient).put(anyString(), any(InputStream.class),
						anyString(), anyInt());
		dbCon.createAttachment("id", "rev", new AttachmentInputStream(
				"attach_id", IOUtils.toInputStream("data"), "whatever"));
	}

	@Test
	public void should_create_db_if_missing() {
		doReturn(HttpResponseStub.valueOf(404,
				"{\"error\":\"not_found\",\"reason\":\"no_db_file\"}"))
						.when(httpClient).head("/test_db/");
		doReturn(HttpResponseStub.valueOf(HttpStatus.CREATED, OK_RESPONSE))
				.when(httpClient).put("/test_db/");
	...
	public void return_all_revisions() {
		doReturn(ResponseOnFileStub.newInstance(200, "revisions.json"))
				.when(httpClient).get("/test_db/some_doc_id?revs_info=true");
		List<Revision> l = dbCon.getRevisions("some_doc_id");
		assertNotNull(l);
		assertEquals(8, l.size());
		assertEquals(
				new Revision("8-8395fd3a7a2dd04022cc1330a4d20e66", "available"),
				l.get(0));
	}
	...
	public void return_attachment_with_open_data_stream() throws Exception {
		ResponseOnFileStub rsp = ResponseOnFileStub.newInstance(200,
				"attachment.txt", "text", 12);
		doReturn(rsp).when(httpClient)
				.get("/test_db/some_doc_id/some_attachment");
		AttachmentInputStream a = dbCon.getAttachment("some_doc_id",
				"some_attachment");
		assertNotNull(a);
		assertFalse(rsp.isConnectionReleased());
		assertEquals("detta är ett påhäng med ett ö i",
				IOUtils.toString(a, "UTF-8"));
	...
	public void should_stream_attachmed_content() {
		doReturn(ResponseOnFileStub.newInstance(200,
				"create_attachment_rsp.json")).when(httpClient).put(anyString(),
						any(InputStream.class), anyString(), anyInt());

		dbCon.createAttachment("docid",
				new AttachmentInputStream("attachment_id",
						IOUtils.toInputStream("content"), "text/html", 12l));

		verify(httpClient).put(eq("/test_db/docid/attachment_id"),
				any(InputStream.class), eq("text/html"), eq(12l));
	}
	...
	public void load_query_result() {
		...
		ViewQuery query = new ViewQuery().dbPath(TEST_DB_PATH)
				.designDocId("_design/testdoc").viewName("test_view")
				.key("key_value");

		doReturn(ResponseOnFileStub.newInstance(200, "view_result.json"))
	...
	public void documents_embedded_in_view_result_should_be_read_directly() {
		...
		doReturn(ResponseOnFileStub.newInstance(200,
				"view_result_with_embedded_docs.json")).when(httpClient)
	...
	public void queries_with_include_doc_should_read_docs_directly() {
		ViewQuery query = new ViewQuery().dbPath(TEST_DB_PATH)
				.designDocId("_design/testdoc").viewName("test_view")
				.includeDocs(true).key("key_value").cacheOk(true);

		doReturn(ResponseOnFileStub.newInstance(200,
				"view_result_with_included_docs.json")).when(httpClient)
						.get(query.buildQuery());
	...
	public void queries_with_ignore_not_found() {
		ViewQuery query = new ViewQuery().dbPath(TEST_DB_PATH)
				.designDocId("_design/testdoc").viewName("test_view")
				.includeDocs(true).keys(Arrays.asList("doc_id0", "doc_id1",
						"doc_id2", "doc_id3", "doc_id4", "doc_id5", "doc_id6"));
		query.setIgnoreNotFound(true);

		doReturn(ResponseOnFileStub.newInstance(200,
				"view_result_with_ignored_docs.json")).when(httpClient)
		...
		verify(httpClient).postUncached(query.buildQuery(),
				query.getKeysAsJson());
	}
	...
	public void dates_should_be_serialized_in_ISO_8601_format()
		...
		DateTime dt = new DateTime(2010, 4, 25, 20, 11, 24, 555,
		...
		doReturn(HttpResponseStub.valueOf(201, json)).when(httpClient)
				.get("/test_db/some_id");
	...
	private void setupPositiveContains(String id) {
		doReturn(HttpResponseStub.valueOf(200, "")).when(httpClient)
				.head("/test_db/" + id);
	}
	...
	public void testGetDbInfo() {
		doReturn(ResponseOnFileStub.newInstance(200, "db_info.json"))
	...
	public void test_replicateTo() {
		...
		when(mockInstance.replicate(any(ReplicationCommand.class)))
				.thenReturn(status);

		StdCouchDbConnector db = new StdCouchDbConnector("test_db",
				mockInstance);
		db.replicateTo(targetDB);
		ArgumentCaptor<ReplicationCommand> ac = ArgumentCaptor
				.forClass(ReplicationCommand.class);
	...
	public void testSetRevsLimit() {
		...
		doReturn(new ReaderInputStream(new StringReader(OK_RESPONSE))).when(rsp)
				.getContent();
	...
	public void test_that_compactView_relases_connection() {
		...
		verify(httpClient).post(eq("/test_db/_compact/_design/someDoc"),
				anyString());
		verify(rsp).releaseConnection();
	}
	...
	public void testGetDesignDocInfo() {
		doReturn(ResponseOnFileStub.newInstance(200, "../design_doc_info.json"))
				.when(httpClient)
	...
	public void testCallUpdateHandler() {
		doReturn(HttpResponseStub.valueOf(201, "response string"))
				.when(httpClient)
		...
		assertEquals("response string", dbCon.callUpdateHandler(
				"_design/designDocID", "functionName", "docID", params));
	}
	...
	public void testCallUpdateHanderWithObject() {
		...
		doReturn(HttpResponseStub.valueOf(201,
				"{\"name\":\"nisse\",\"age\":12,\"_id\":\"some_id\",\"_rev\":\"123D123\"}"))
						.when(httpClient)
						.put("/test_db/_design/designDocID/_update/functionName/docID",
								"{\"value\":\"value\",\"param\":\"param\"}");

		UpdateHandlerRequest req = new UpdateHandlerRequest();
		req.designDocId("_design/designDocID").functionName("functionName")
				.docId("docID").body(new TestRequest("param", "value"));
	...
	public void testEnsureFullCommit() {
		doReturn(HttpResponseStub.valueOf(200,
				"{\"ok\" : true, \"instance_start_time\" : \"1288186189373361\"}"))
						.when(httpClient)
						.post("/test_db/_ensure_full_commit", "");
	...
	public void testPurge() {
		...
		revisionsToPurge.put("Billy", Collections
				.singletonList("17-b3eb5ac6fbaef4428d712e66483dcb79"));
	...
	public void updateMultipart_should_perform_put_operation_with_path_set_to_db_followed_by_id() {
		...
		doReturn(null).when(httpClient).put(eq(expectedPath),
				any(InputStream.class), anyString(), anyLong());

		dbCon.updateMultipart(id, null, "abc", 0, null);

		verify(httpClient).put(eq(expectedPath), any(InputStream.class),
				anyString(), anyLong());
	}
	...
	public void updateMultipart_should_perform_put_operation_with_the_given_InputStream() {
		...
		doReturn(null).when(httpClient).put(anyString(), eq(stream),
				anyString(), anyLong());
	...
	public void updateMultipart_should_perform_put_operation_with_content_type_set_to_multipart_related_with_boundary() {
		...
		String expectedContentType = "multipart/related; boundary=\"" + boundary
				+ "\"";
		doReturn(null).when(httpClient).put(anyString(), any(InputStream.class),
				eq(expectedContentType), anyLong());
		dbCon.updateMultipart("a", null, boundary, 0, null);
		verify(httpClient).put(anyString(), any(InputStream.class),
				eq(expectedContentType), anyLong());
	}
	...
	public void updateMultipart_should_perform_put_operation_with_content_type_set_to_length() {
		...
		verify(httpClient).put(anyString(), any(InputStream.class), anyString(),
				eq(length));
	}
	...
	public void update_with_stream_should_perform_put_operation_with_content_type_set_to_application_json() {
		...
		doReturn(null).when(httpClient).put(expectedPath, inputStream,
				expectedContentType, documentLength);
		dbCon.update(documentId, inputStream, documentLength, options);

		verify(httpClient).put(expectedPath, inputStream, expectedContentType,
				documentLength);
	}
	...
	public void testCopy() {
		...
		doReturn(HttpResponseStub.valueOf(201, OK_RESPONSE_WITH_ID_AND_REV))
				.when(httpClient).copy(anyString(), anyString());
	...
	public void testUpdateSecurityConfigurationValue() {
		doReturn(HttpResponseStub.valueOf(200, "{\"ok\":true}"))
				.when(httpClient).put(anyString(), anyString());
	...
	protected static void assertEqualJson(String expectedFileName,
			Charset expectedFileCharset, String actual) throws IOException {
		Reader expectedReader = new StringReader(
				getString(expectedFileName, expectedFileCharset));
		Reader actualReader = new StringReader(actual);
		assertTrue(format("expected: %s was: %s",
				getString(expectedFileName, expectedFileCharset), actual),
				JSONComparator.areEqual(expectedReader, actualReader));
	}

	protected static void assertEqualJson(String expectedFileName,
			Charset expectedFileCharset, byte[] actual) throws IOException {
		Reader expectedReader = new StringReader(
				getString(expectedFileName, expectedFileCharset));
		Reader actualReader = new InputStreamReader(
				new ByteArrayInputStream(actual), Charset.forName("UTF-8"));
		assertTrue(
				format("expected: %s was: %s",
						getString(expectedFileName, expectedFileCharset),
						new String(actual, Charset.forName("UTF-8"))),
				JSONComparator.areEqual(expectedReader, actualReader));
	}

	protected static String getString(String resourceName, Charset charset)
			throws IOException {
		InputStream inputStream = null;
		Reader reader = null;
		try {
			inputStream = StdCouchDbConnectorTest.class
